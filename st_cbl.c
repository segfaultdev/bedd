...
int st_cbl_color(int prev_state, int *state, const char *text, int length) {
  if (prev_state == st_cbl_line_comment) {
    if (text[0] == '\n') {
      *state = st_cbl_default;
      return st_color_default;
    }
    return st_color_comment;
  }

  if (prev_state == st_cbl_string) {
    if (text[0] == '"') {
      *state = st_cbl_default;
    }
  ...
  if (prev_state == st_cbl_default) {
    if (text[0] == '*') {
      *state = st_cbl_line_comment;
      return st_color_comment;
    } else if (isdigit(text[0])) {
      *state = st_cbl_number;
      return st_color_number;
    } else if (is_ident(text[0])) {
      int is_func = 0, is_keyword = 0;
      int ident_length = 1;
      ...
      for (int i = 0; i < sizeof(keywords) / sizeof(keywords[0]); i++) {
        if (strlen(keywords[i]) == ident_length)
          if (strncmp(keywords[i], buffer, ident_length) == 0) {
            is_keyword = 1;
            break;
          }
      }
