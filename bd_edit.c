...
static const int __edit_count = 7;
...
void bd_edit_draw(bd_view_t *view) {
  ...
  io_cursor(2, 8);
  io_printf(IO_BOLD "%sScroll step (with mouse wheel, default 2):" IO_NORMAL " %d", (index == 2 ? IO_INVERT : ""), bd_config.scroll_step);
  
  io_cursor(2, 9);
  io_printf(IO_BOLD "%sHorizontal scroll margin (default 0):" IO_NORMAL " %d", (index == 3 ? IO_INVERT : ""), bd_config.scroll_width_margin);
  
  io_cursor(2, 11);
  io_printf(IO_BOLD "%sCharacters per undo entry (default 48):" IO_NORMAL " %d", (index == 4 ? IO_INVERT : ""), bd_config.undo_edit_count);
  
  io_cursor(2, 12);
  io_printf(IO_BOLD "%sMaximum undo entries (default 64):" IO_NORMAL " %d", (index == 5 ? IO_INVERT : ""), bd_config.undo_depth);
  
  io_cursor(2, 14);
  io_printf(IO_BOLD "%sTheme:" IO_NORMAL " \"%s\" (%d/%d)", (index == 6 ? IO_INVERT : ""), theme_names[bd_config.theme], bd_config.theme + 1, theme_count);
