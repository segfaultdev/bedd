...
enum {
  ...
  st_cobol_string,
  st_cobol_char,
  st_cobol_string_escape,
  st_cobol_char_escape,
  st_cobol_line_comment,
};
...
int st_cobol_color(int prev_state, int *state, const char *text, int length) {
  if (prev_state == st_cobol_line_comment) {
    if (text[0] == '\n') {
  ...
  if (prev_state == st_cobol_default) {
    if (text[0] == '*') {
      *state = st_cobol_line_comment;
      return st_color_comment;
    } else if (isdigit(text[0])) {
      *state = st_cobol_number;
      ...
      for (int i = 0; i < (int)(sizeof(keywords) / sizeof(const char *)); i++) {
        if ((int)(strlen(keywords[i])) == ident_length)
          if (!strncmp(keywords[i], buffer, ident_length)) {
            is_keyword = 1;
            break;
          }
      }
