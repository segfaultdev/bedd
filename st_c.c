  ...
  if (prev_state == st_c_default) {
    if (isdigit(text[0])) {
      *state = st_c_number;
      ...
      if (ident_length == 4 && strstr("auto,case,char,else,enum,goto,long,void", buffer)) is_keyword = 1;
      if (ident_length == 5 && strstr("break,class,const,float,short,union,using,while", buffer)) is_keyword = 1;
